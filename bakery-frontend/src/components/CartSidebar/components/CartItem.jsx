...
function CartItem({ item, onUpdate, onRemove }) {
  const price = (Number(item.base_price) + Number(item.price_adjustment)).toLocaleString();
    ...
    >
      ...
      <div className="cart-item-info">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: 10 }}>
          <span className="cart-item__price">{price}đ</span>
          <div className="qty-control">
            <button type="button" className="qty-btn" onClick={() => onUpdate(item.cart_item_id, item.quantity - 1, item.stock_quantity)}>
              <Minus size={14} />
            </button>
